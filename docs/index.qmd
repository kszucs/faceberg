...
```bash
faceberg user/mycatalog add stanfordnlp/imdb --table default.movies
```
...
Read tables directly into pandas DataFrames using `pd.read_iceberg()`.

**Via REST catalog** (recommended for remote catalogs):
...
df = pd.read_iceberg(
    ...
    catalog_properties={
        "type": "rest",
        "uri": "https://user-mycatalog.hf.space",
    },
    columns=["text", "label"],
    limit=10,
)
...
**Via Faceberg catalog** (supports both local and remote):
...
df = pd.read_iceberg(
    ...
    catalog_name="faceberg",
    catalog_properties={
        "py-catalog-impl": "faceberg.catalog.RemoteCatalog",
        "uri": "hf://spaces/user/mycatalog",
    },
    columns=["text", "label"],
    limit=10,
)
