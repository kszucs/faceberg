...
if __name__ == "__main__":
    ...
    print("\n✅ All basic JsonCatalog operations work!")

    # Test FacebergCatalog
    print("\n" + "=" * 60)
    print("Testing FacebergCatalog...")
    print("=" * 60)

    import tempfile

    with tempfile.TemporaryDirectory() as tmpdir:
        print("\nCreating FacebergCatalog from config...")
        config = FacebergConfig(
            catalog=CatalogConfig(name="test_faceberg", location=tmpdir),
            datasets=[DatasetConfig(name="imdb", repo="stanfordnlp/imdb", configs=None)],
        )

        faceberg = FacebergCatalog.from_config(config)
        print("✓ Created FacebergCatalog from config")

        faceberg.initialize()
        print("✓ Initialized FacebergCatalog")

        print("\n✅ All basic FacebergCatalog operations work!")
